...
Move AiPlayer::GetNextMove(const Omok& omok) {
	return GetNextMoveWithPrint(omok);
}

Move AiPlayer::GetNextMoveWithPrint(const Omok& omok) {
	// AiPlayer가 black인 경우 중앙/혹은 랜덤으로 first move 후에 진행
	if (omok.GetMoveCount() == 0) {
		return GetFirstMove(omok.GetSize());
	}

	std::ofstream fout("root2_mcts_info.txt", std::ios::app);
	...
	// Move next_move = first_tree->GetBestChildIndex();
	// Voting Algorithm, 가장 많은 표를 받은 수를 선택
